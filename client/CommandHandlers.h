...
#define HANDLER_HEADER(HANDLER_NAME, COMMAND_ID) \
	public: HANDLER_NAME() : CommandHandler(COMMAND_ID) {} \
	virtual void OnResponse(byte * data, uint size)



class PingResponse : public CommandHandler
{
...
class OneWireEnumBegin : public CommandHandler
{
	HANDLER_HEADER(OneWireEnumBegin, RSP_ONE_WIRE_ENUM_BEGIN)
	{
		g_commMgr->ClearOneWireDeviceList();
	}
};


class OneWireRomFound : public CommandHandler
{
	HANDLER_HEADER(OneWireRomFound, RSP_ONE_WIRE_ROM_FOUND)
	{
		...
		*/
		...
		g_commMgr->AddOneWireDevice(OneWireAddr(data));
...
class OneWireEnumEnd : public CommandHandler
{
	HANDLER_HEADER(OneWireEnumEnd, RSP_ONE_WIRE_ENUM_END)
	{
		g_commMgr->OnOneWireEnumerated();
	}
};


class OneWireTemperature : public CommandHandler
{
...
class ReadEEPROM : public CommandHandler
{
...
class WriteEEPROM : public CommandHandler
{
